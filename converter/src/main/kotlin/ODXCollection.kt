...
class ODXCollection(val data: Map<String, ODX>) {
    ...
    val compuScales: Set<COMPUSCALE> by lazy {
        val data = combinedDataObjectProps.values
            .filterIsInstance<DATAOBJECTPROP>()
            .flatMap {
                (it.compumethod.compuinternaltophys.compuscales?.compuscale ?: emptyList()) +
                        (it.compumethod.compuphystointernal.compuscales.compuscale ?: emptyList())
            }
        data.toSet()
    }
}