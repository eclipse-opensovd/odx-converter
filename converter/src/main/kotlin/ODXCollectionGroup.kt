...
class EcuGroup(
    ...
    val revision: String?,
    /**
     * The BASE-VARIANT this group is anchored on, or `null` for the synthetic
     * "functional_groups" group used when a description contains no
     * BASE-VARIANT at all (a functional-groups-only PDX).
     */
...
class ODXCollectionGroup(
     ...
     * container short-name index [collectionsByContainer] first and falling back
     * to the layer-level [layerNameToCollection] index (for `DOCTYPE=LAYER` refs).
     */
    private fun collectionForDocref(docref: String): ODXCollection? = collectionsByContainer[docref] ?: layerNameToCollection[docref]
    ...
    val ecuGroups: List<EcuGroup> by lazy {
        ...
        // A description with no BASE-VARIANT at all (a functional-groups-only
        // PDX) would otherwise produce zero ECU groups, making its data
        // unreachable (no `ECUInfo` entry to identify it by). Emit a single
        // synthetic group so the shared functional groups remain reachable
        // under a stable, well-known name.
        if (groups.isEmpty() && functionalGroups.isNotEmpty()) {
            listOf(
                EcuGroup(
                    name = "functional_groups",
                    revision = functionalGroups.firstOrNull()?.let { revisionFor(it) },
                    baseVariant = null,
                    ecuVariants = emptyList(),
                ),
            )
        } else {
            groups
